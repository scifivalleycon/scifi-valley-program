...
async function refresh(force=false){try{const response=await fetch(`data/map-settings.json?layout=${Date.now()}`,{cache:"no-store"});if(!response.ok)return;const raw=await response.json();const settings=Array.isArray(raw)?raw[0]:raw;const layout=settings?.homeLayout;const items=normalize(layout);const signature=JSON.stringify(items);if(signature!==lastSignature){lastSignature=signature;apply(items)}}catch(err){console.warn("SFVC homepage layout could not refresh",err)}}
